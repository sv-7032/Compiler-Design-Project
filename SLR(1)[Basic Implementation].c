...
void getRemainingBuffer(char rawInput[][MAX_STR], int currentIdx, int totalTokens, char* bufferOut) {
    bufferOut[0] = '\0';
    for (int i = currentIdx; i < totalTokens; i++) {
        strcat(bufferOut, rawInput[i]);
    }
}

// Lexical Scanner: Converts raw string like "(a+b)*c" into structured token arrays
int lexer(const char* inputStr, char rawInput[][MAX_STR], char tokens[][MAX_STR]) {
    int i = 0;
    int count = 0;

    while (inputStr[i] != '\0' && inputStr[i] != '\n') {
        // Skip whitespace
        ...
        if (inputStr[i] == '+' || inputStr[i] == '*' || inputStr[i] == '(' || inputStr[i] == ')') {
            rawInput[count][0] = inputStr[i];
            rawInput[count][1] = '\0';

            tokens[count][0] = inputStr[i];
            tokens[count][1] = '\0';

            count++;
            i++;
        }
        // Identifiers / Numbers (multi-character variables like "val" or "10")
        else if (isalnum(inputStr[i])) {
            int len = 0;
            while (isalnum(inputStr[i])) {
                rawInput[count][len++] = inputStr[i++];
            }
            rawInput[count][len] = '\0';
            strcpy(tokens[count], "id"); // Map variable name to grammar symbol 'id'
            count++;
        }
        else {
...
int main(void) {
    ...
    if (fgets(inputBuf, sizeof(inputBuf), stdin) == NULL) return 1;

    char rawInput[MAX_TOKENS][MAX_STR];
    char tokens[MAX_TOKENS][MAX_STR];

    int totalTokens = lexer(inputBuf, rawInput, tokens);
    ...
    printf("\n=== SHIFT-REDUCE PARSING & INTERMEDIATE CODE GENERATION (TAC) ===\n");
    printf("%-5s %-12s %-20s %-24s %-20s\n", "Step", "State Stack", "Input Buffer", "Action", "Generated TAC");
    printf("--------------------------------------------------------------------------------------------------\n");

    while (1) {
        int currentState = stack[top].state;
        const char* currentToken = tokens[tokenIdx];
        
        char remainingBuffer[128];
        getRemainingBuffer(rawInput, tokenIdx, totalTokens, remainingBuffer);

        const char* action = getAction(currentState, currentToken);

        printf("%-5d State %-6d %-20s ", step++, currentState, remainingBuffer);
        ...
        if (action[0] == 'S') {
            ...
            strcpy(stack[top].val, rawInput[tokenIdx]);
